...
class SpacyMultiEntityFineTuner:
    ...
    def __init__(self, model_size: str = "sm", lang: str = "fr"):
        ...
        self.custom_entities = {}  # Ex: {'RNE': [...], 'CODE_POSTAL': [...]}
        self.custom_entity_patterns = {}  # Patterns regex pour validation
    ...
    def load_custom_entities_from_file(self, filepath: str, entity_type: str, 
                                     pattern: Optional[str] = None) -> List[str]:
        """
        Charge des entités personnalisées depuis un fichier
        
        Args:
            filepath: Chemin vers le fichier d'entités
            entity_type: Type d'entité personnalisée (ex: 'RNE', 'CODE_POSTAL')
            pattern: Pattern regex optionnel pour valider le format
        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            for line in f:
                entity = line.strip()
                if entity:
                    # Valider avec le pattern si fourni
                    if pattern:
                        if re.match(pattern, entity):
                            entities.append(entity)
                        else:
                            print(f"⚠ Entité ignorée (ne correspond pas au pattern): {entity}")
                    else:
                        entities.append(entity)
        
        self.custom_entities[entity_type] = entities
        if pattern:
            self.custom_entity_patterns[entity_type] = pattern
        
        print(f"✓ {len(entities)} entités de type {entity_type} chargées")
        return entities
    ...
    def load_sentence_templates(self, filepath: str) -> List[str]:
        ...
        - {LIEU} pour les lieux
        - {CUSTOM_TYPE} pour les entités personnalisées (ex: {RNE})
    ...
    def generate_training_data(self, templates: List[str], 
        ...
        # Créer un mapping unifié de tous les placeholders
        placeholder_mapping = {
            'NOM': ('PER', self.entities.get('PER', [])),
            'ORG': ('ORG', self.entities.get('ORG', [])),
            'LIEU': ('LOC', self.entities.get('LOC', []))
        }
        
        # Ajouter les entités personnalisées
        for custom_type, custom_list in self.custom_entities.items():
            placeholder_mapping[custom_type] = (custom_type, custom_list)
            print(f"  → Ajout du placeholder {custom_type} avec {len(custom_list)} entités")
        
        print(f"\nPlaceholders disponibles: {', '.join(placeholder_mapping.keys())}")
        
        for template in templates:
            # Identifier tous les placeholders dans le template
            placeholders_info = []
            
            # Pattern pour capturer tous les types de placeholders
            pattern = r'\{([A-Z_]+)\}'
            
            for match in re.finditer(pattern, template):
                placeholder_name = match.group(1)
                
                # Vérifier si le placeholder est reconnu
                if placeholder_name in placeholder_mapping:
                    entity_type, entity_list = placeholder_mapping[placeholder_name]
                    ...
                    print(f"⚠ Placeholder non reconnu: {{{placeholder_name}}}")
            ...
            for _ in range(samples_per_template):
                ...
                for ph_info in placeholders_info:
                    entity_list = ph_info['entity_list']
                    
                    # Vérifier qu'il y a des entités disponibles
                    if not entity_list:
                        continue
                    ...
                    entities.append((start, end, ph_info['type']))
...
class SpacyMultiEntityFineTunerGUI:
    ...
    def __init__(self):
        ...
        # Variables pour entités standards
        self.names_file = tk.StringVar()
        self.orgs_file = tk.StringVar()
        self.places_file = tk.StringVar()
        self.templates_file = tk.StringVar()
        
        # Variables pour entités personnalisées
        self.custom_entities_vars = []  # Liste de tuples (name_var, file_var, pattern_var)
    ...
    def setup_ui(self):
        ...
        custom_frame = ttk.LabelFrame(main_frame, text="Entités personnalisées", padding="10")
        custom_frame.pack(fill="x", pady=10, padx=5)
        
        custom_info = ttk.Label(custom_frame, 
                               text="Ajoutez vos propres types d'entités (ex: RNE, CODE_POSTAL, etc.)",
                               font=('Helvetica', 9, 'italic'))
        custom_info.pack(pady=5)
        
        # Frame pour les entités personnalisées
        self.custom_entities_frame = ttk.Frame(custom_frame)
        self.custom_entities_frame.pack(fill="x", pady=5)
        
        # Bouton pour ajouter une entité personnalisée
        add_custom_button = ttk.Button(custom_frame, text="➕ Ajouter une entité personnalisée",
                                     command=self.add_custom_entity)
        ...
        template_info = ttk.Label(templates_frame, 
                                 text="Utilisez {NOM} pour les personnes, {ORG} pour les organisations, {LIEU} pour les lieux\n"
                                      "Pour les entités personnalisées, utilisez le nom exact en majuscules (ex: {RNE}, {INE})",
                                 font=('Helvetica', 9, 'italic'))
    ...
    def add_custom_entity(self):
        """Ajoute un champ pour une entité personnalisée"""
        # Créer les variables
        name_var = tk.StringVar()
        file_var = tk.StringVar()
        pattern_var = tk.StringVar()
        
        # Frame pour cette entité
        entity_frame = ttk.Frame(self.custom_entities_frame)
        entity_frame.pack(fill="x", pady=5)
        
        # Nom de l'entité
        ttk.Label(entity_frame, text="Type:").pack(side="left", padx=5)
        name_entry = ttk.Entry(entity_frame, textvariable=name_var, width=15)
        name_entry.pack(side="left", padx=5)
        name_entry.insert(0, "RNE")  # Exemple par défaut
        
        # Pattern (optionnel)
        ttk.Label(entity_frame, text="Pattern:").pack(side="left", padx=5)
        pattern_entry = ttk.Entry(entity_frame, textvariable=pattern_var, width=20)
        pattern_entry.pack(side="left", padx=5)
        pattern_entry.insert(0, r"^\d{7}[A-Z]$")  # Exemple RNE
        
        # Fichier
        ttk.Label(entity_frame, text="Fichier:").pack(side="left", padx=5)
        ttk.Entry(entity_frame, textvariable=file_var, width=30).pack(side="left", padx=5)
        
        # Boutons
        ttk.Button(entity_frame, text="Parcourir",
                  command=lambda: self.browse_file(file_var)).pack(side="left", padx=5)
        
        def remove_entity():
            entity_frame.destroy()
            self.custom_entities_vars.remove((name_var, file_var, pattern_var))
        
        ttk.Button(entity_frame, text="❌", width=3,
                  command=remove_entity).pack(side="left", padx=5)
        
        # Ajouter aux variables
        self.custom_entities_vars.append((name_var, file_var, pattern_var))
    ...
    def generate_example_files(self):
        ...
        try:
            ...
            # RNE (exemple d'entité personnalisée)
            with open(f"{directory}/exemples_rne.txt", "w", encoding="utf-8") as f:
                f.write("0751234A\n")
                f.write("0692345B\n")
                f.write("0133456C\n")
                f.write("0754567D\n")
            ...
            # INE (autre exemple d'entité personnalisée)
            with open(f"{directory}/exemples_ine.txt", "w", encoding="utf-8") as f:
                f.write("0123456789A\n")
                f.write("9876543210B\n")
                f.write("1234567890C\n")
                f.write("0987654321D\n")
            ...
            # Phrases types avec entités personnalisées
            with open(f"{directory}/exemples_phrases_types_avancees.txt", "w", encoding="utf-8") as f:
                f.write("# Phrases avec entités standards\n")
                ...
                f.write("\n# Phrases avec entités personnalisées (RNE)\n")
                f.write("L'établissement {RNE} accueille {NOM} comme nouveau directeur.\n")
                f.write("Le code RNE {RNE} correspond à {ORG} situé à {LIEU}.\n")
                f.write("{NOM} enseigne dans l'établissement {RNE}.\n")
                f.write("La réunion pour l'établissement {RNE} est organisée par {NOM}.\n")
                f.write("Les résultats de l'établissement {RNE} ont été publiés par {ORG}.\n")
                f.write("\n# Phrases avec entités personnalisées (INE)\n")
                f.write("L'étudiant avec l'INE {INE} est inscrit à {ORG}.\n")
                f.write("{NOM} a pour identifiant national étudiant {INE}.\n")
                f.write("Le dossier de l'étudiant {INE} a été transféré à {LIEU}.\n")
                f.write("L'INE {INE} correspond à {NOM} inscrit en {ORG}.\n")
                f.write("\n# Phrases mixtes avec plusieurs entités personnalisées\n")
                f.write("L'étudiant {INE} est affecté à l'établissement {RNE}.\n")
                f.write("{NOM} (INE: {INE}) étudie dans l'établissement {RNE} à {LIEU}.\n")
                f.write("Le transfert de l'étudiant {INE} vers l'établissement {RNE} a été validé par {ORG}.\n")
    ...
    def start_training(self):
        ...
        # Vérifier les entités personnalisées valides
        valid_custom_entities = []
        for name_var, file_var, pattern_var in self.custom_entities_vars:
            if name_var.get() and file_var.get():
                valid_custom_entities.append((name_var, file_var, pattern_var))
        
        # Vérifier qu'on a au moins une source d'entités
        has_standard_entities = any([self.names_file.get(), self.orgs_file.get(), self.places_file.get()])
        has_custom_entities = len(valid_custom_entities) > 0
        
        if not has_standard_entities and not has_custom_entities:
            messagebox.showerror("Erreur", 
                               "Veuillez sélectionner au moins un fichier d'entités (standard ou personnalisé)!")
            return
    ...
    def run_training(self):
        ...
        try:
            ...
            # Charger les entités personnalisées
            for name_var, file_var, pattern_var in self.custom_entities_vars:
                entity_name = name_var.get()
                entity_file = file_var.get()
                entity_pattern = pattern_var.get()
                
                if entity_name and entity_file:
                    self.log(f"\nChargement des entités {entity_name}...")
                    tuner.load_custom_entities_from_file(
                        entity_file, 
                        entity_name,
                        entity_pattern if entity_pattern else None
                    )
            
            # Charger les templates
            self.log("\nChargement des phrases types...")
        ...
        try:
            ...
            def analyze():
                ...
                for ent in sorted(doc.ents, key=lambda x: x.start_char):
                    ...
                    # Ajouter l'entité annotée avec couleurs étendues
                    entity_colors = {
                        'PER': '🔵', 
                        'ORG': '🟢', 
                        'LOC': '🔴',
            ...
            examples = [
                ...
                "L'établissement 0751234A accueille Catherine Rousseau comme nouvelle directrice.",
                "Le code RNE 0692345B correspond au Lycée Victor Hugo situé à Lyon.",
                "La réunion pour l'établissement 0133456C est organisée par Marie Dupont."
            ]
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        parser.add_argument("--custom-entity", nargs=3, action='append', metavar=('NAME', 'FILE', 'PATTERN'),
                           help="Entité personnalisée: nom fichier pattern(optionnel, mettre '' si pas de pattern)")
        ...
        has_entities = any([args.names_file, args.orgs_file, args.places_file, args.custom_entity])
        if not has_entities:
            print("❌ Erreur: Au moins un fichier d'entités doit être fourni!")
            print("\nExemples d'utilisation:")
            print("  # Avec entités standards:")
            print("  python script.py --names-file noms.txt --templates-file phrases.txt")
            print("\n  # Avec entités personnalisées:")
            print("  python script.py --custom-entity RNE rne_list.txt '^\\d{7}[A-Z] --templates-file phrases.txt")
            sys.exit(1)
        ...
        if args.custom_entity:
            for entity_name, entity_file, pattern in args.custom_entity:
                # Traiter le pattern vide
                pattern = pattern if pattern and pattern != "''" else None
                tuner.load_custom_entities_from_file(entity_file, entity_name, pattern)
        ...
        test_texts = [
            ...
            "L'établissement 0751234A est dirigé par Jean Martin.",
            "Le code RNE 0692345B correspond au lycée de Toulouse."
        ]
        tuner.test_model(test_texts)
        ...
        print("doc = nlp('Marie travaille dans l\\'établissement 0751234A.')")
        print("for ent in doc.ents:")
        print("    print(f'{ent.text} ({ent.label_})')")
    else:
